    ...
    const p = new Promise<WorkerRpcReturnValue>((resolve, reject) =>
        wsClient.onMessage.addListener((data) => {
            ...
            if ('error' in parsed) {
                ...
                console.log(
                    'Request failed: ' + JSON.stringify(transaction, null, 2)
                )
            ...
            if (res.status.isError) {
                console.log(
                    'Rpc response error: ' + decodeRpcBytesAsString(res.value)
                )
            }

            if (
                res.status.isTrustedOperationStatus &&
                res.status.asTrustedOperationStatus[0].isInvalid
            ) {
                console.log(
                    'Rpc trusted operation execution failed, hash: ',
                    res.value.toHex()
                )
...
export const createPublicGetter = (
    parachainApi: ApiPromise,
...
export const getSidechainNonce = async (
    ...
    if (nonceHex) {
        nonce = context.api
            .createType(
                'Index',
                '0x' + nonceHex.slice(2)?.match(/../g)?.reverse().join('')
            )
            .toNumber()
    }
...
export const getIdGraphHash = async (
    ...
    const hash = context.api
        .createType('Option<Bytes>', hexToU8a(res.value.toHex()))
        .unwrap()
...
export const sendRequestFromTrustedCall = async (
    ...
    const trustedOperation = context.api.createType('TrustedOperation', {
        direct_call: call,
    })
    console.log(
        'trustedOperation: ',
        JSON.stringify(trustedOperation.toHuman(), null, 2)
    )
...
export const createRsaRequest = async (
    ...
    return parachainApi
        .createType('RsaRequest', { shard: hexToU8a(mrenclave), payload })
        .toU8a()
}
...
export const createAesRequest = async (
    ...
    const encryptedAesKey = compactAddLength(
        bufferToU8a(encryptWithTeeShieldingKey(teeShieldingKey, aesKey))
    )
    return parachainApi
...
export function decodeIdGraph(sidechainRegistry: TypeRegistry, value: Bytes) {
    const idgraphBytes = sidechainRegistry.createType(
        'Option<Bytes>',
        hexToU8a(value.toHex())
    )
...
export function getTopHash(parachainApi: ApiPromise, call: TrustedCallSigned) {
    const trustedOperation = parachainApi.createType('TrustedOperation', {
        direct_call: call,
    })
