...
import { AesOutput, CorePrimitivesIdentity } from '@litentry/parachain-api'
...
export function decryptWithAes(
    ...
    const secretKey = crypto.createSecretKey(hexToU8a(key))
    const tagSize = 16
    const ciphertext = aesOutput.ciphertext
...
export class PolkadotSigner implements Signer {
    ...
    getIdentity(
        ...
        return buildIdentityHelper(
            u8aToHex(this.getAddressRaw()),
            'Substrate',
            context
        )
    }
