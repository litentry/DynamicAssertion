...
function restart_worker_services {
  ...
  if [ "$PRODUCTION" = "true" ]; then
    ...
    # staging example
    commands=(
      "--clean-reset -T ws:localhost -P 2000 -p 9944 -r 3443 -w 2001 -h 4545 --parentchain-start-block 0 run --skip-ra --dev"
    )
  fi
  docker run -d \
      ...
      --restart always \
      --net=host \
      --device=/dev/null:/dev/sgx/provision \
      --device=/dev/null:/dev/sgx/enclave \
