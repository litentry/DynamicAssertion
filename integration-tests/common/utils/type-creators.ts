...
import type { LitentryMultiSignature } from '@litentry/parachain-api'
...
export async function createLitentryMultiSignature(
    ...
    if (payload instanceof Uint8Array) {
        const signature = await signer.sign(
            signerType === 'bitcoin' ? u8aToHex(payload).substring(2) : payload
        )
    ...
    if (payload.startsWith('0x')) {
        const signature = await signer.sign(
            signerType === 'bitcoin' ? payload.substring(2) : hexToU8a(payload)
        )
    ...
    const signature = await signer.sign(
        signerType === 'bitcoin' ? payload : stringToU8a(payload)
    )
