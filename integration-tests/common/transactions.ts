...
export const subscribeToEventsWithExtHash = async (
    ...
    return new Promise<FrameSystemEventRecord[]>((resolve, reject) => {
        ...
        */
        const unsubscribe = context.api.rpc.chain.subscribeNewHeads(
            async (blockHeader) => {
                ...
                if (matchingEvent.length == 0) {
                    ...
                    if (blocksToScan < 1) {
                        reject(
                            new Error(
                                `timed out listening for reqExtHash: ${requestIdentifier} in parachain events`
                            )
                        )
                        ;(await unsubscribe)()
                    }
    ...
    return new Promise<void>((resolve, reject) => {
        const unsubscribe = api.rpc.chain.subscribeNewHeads(async (header) => {
            ...
            if (++count === blocksToCheck) {
                ...
                reject(
                    new Error(
                        `Timeout: Block #${blockNumber} not reached within ${blocksToCheck} blocks.`
                    )
                )
            }
        })
...
export async function setAliceAsAdmin(api: ApiPromise) {
    ...
    const tx = await sudoWrapperGC(
        api,
        api.tx.teebag.setAdmin(
            'esqZdrqhgH8zy1wqYh1aLKoRyoRWLFbX9M62eKfaTAoK67pJ5'
        )
    )
...
export async function sudoWrapperGC(
    api: ApiPromise,
    tx: SubmittableExtrinsic<ApiTypes>
) {
    ...
    if (chain != 'rococo-dev') {
        ...
        return api.tx.council.propose(
            threshold,
            call,
            api.createType('Compact<u32>', tx.length)
        )
    } else {
...
export async function setScheduledEnclave(
    api: ApiPromise,
    block: number,
    mrenclave: string
) {
    ...
    const tx = api.tx.teebag.setScheduledEnclave(
        'Identity',
        block,
        hexToU8a(`0x${mrenclave}`)
    )
